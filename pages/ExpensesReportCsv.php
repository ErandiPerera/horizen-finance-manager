...
$GetExpenseHistory = "SELECT BillsId, Title, Dates,CategoryName, bills.AccountId,AccountName, Amount, Description from bills left join 
category on bills.CategoryId = category.CategoryId left join account on bills.AccountId = account.AccountId where bills.UserId = $UserId ORDER BY bills.Dates DESC";
...
// Filter Report Expense
	$GetExpenseHistory = "SELECT BillsId, Title, Dates,CategoryName, bills.AccountId,AccountName, Amount, Description from bills left join 
  category on bills.CategoryId = category.CategoryId left join account on bills.AccountId = account.AccountId where 
...
//make data download instead of display the data
